...
TEST(Clip, gdip_clip_containers)
{
	STARTUP
...
TEST(Clip, gdip_clip_path )
{
	...
	GpRect rc = { 100, 100, 50, 50 };
	GpPath *path;
	C (GdipCreatePath (FillModeAlternate, &path));
	{
		C (GdipAddPathRectangleI (path, rc.X, rc.Y, rc.Width, rc.Height));
	...
	for (int i = 0; i < sizeof (points) / sizeof (points[0]); i += 2) {
		...
		assertEqualARGB (color, expectedColor);
	}
