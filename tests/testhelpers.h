...
#define ASSERT_FLOAT_EQ_NAN(val1, val2) \
{ \
    if (std::isnan (val1)) \
    { \
        ASSERT_TRUE (std::isnan (val2)); \
        \
    } \
    else \
    { \
...
{ \
 ...
 \
    ...
    ASSERT_EQ (Ok, status); \
    ASSERT_EQ (expectedPixelFormat, pixelFormat); \
 \
    status = GdipGetImageWidth (image, &width); \
    ASSERT_EQ (Ok, status); \
    ASSERT_EQ (expectedWidth, width); \
 \
    status = GdipGetImageHeight (image, &height); \
    ASSERT_EQ (Ok, status); \
    ASSERT_EQ (expectedHeight, height); \
 \
    status = GdipGetImageBounds (image, &bounds, &unit); \
    ASSERT_EQ (Ok, status); \
    ASSERT_FLOAT_EQ_NAN (expectedX, bounds.X); \
    ASSERT_FLOAT_EQ_NAN (expectedY, bounds.Y); \
    ASSERT_FLOAT_EQ_NAN (expectedBoundsWidth, bounds.Width); \
    ASSERT_FLOAT_EQ_NAN (expectedBoundsHeight, bounds.Height); \
 ...
 \
    ...
    if (fabsf (dimensionWidth - expectedDimensionWidth) > 0.05) \
        ASSERT_FLOAT_EQ_NAN (expectedDimensionWidth, dimensionWidth); \
    if (fabsf (dimensionHeight - expectedDimensionHeight) > 0.05) \
        ASSERT_FLOAT_EQ_NAN (expectedDimensionHeight, dimensionHeight); \
 \
 ...
 \
    ...
    if (WINDOWS_GDIPLUS) \
    { \
        ASSERT_EQ (expectedPropertyCount, propertyCount); \
    } \
}
...
{ \
 ...
 \
    ...
    ASSERT_EQ (Ok, status); \
 \
    ASSERT_EQ (flags, palette->Flags); \
    ASSERT_EQ ((int) (sizeof (entries) / sizeof (ARGB)), palette->Count); \
 \
...
{ \
 ...
 \
    ...
    status = GdipGetImagePalette (image, palette, size); \
    ASSERT_EQ (Ok, status); \
    ASSERT_EQ (sizeof (ColorPalette), size); \
 \
    ASSERT_EQ (flags, palette->Flags); \
    ASSERT_EQ (0, palette->Count); \
    ASSERT_EQ (0x123, palette->Entries[0]); \
 \
    GdipFree (palette); \
}
...
ATTRIBUTE_USED static void verifyRegionImpl(GpRegion *region, float expectedX, float expectedY, float expectedWidth, float expectedHeight, BOOL expectedIsEmpty, BOOL expectedIsInfinite, const char *file, const char *function, int line)
{
    ...
    ASSERT_EQ (Ok, status);
    ASSERT_EQ (expectedIsEmpty, isEmpty);

    status = GdipIsInfiniteRegion (region, graphics, &isInfinite);
    ASSERT_EQ (Ok, status);
    ASSERT_EQ (expectedIsInfinite, isInfinite);
...
ATTRIBUTE_USED static void verifyRegionScansImpl (GpRegion *region, RectF *expectedScans, INT expectedCount, const char *file, const char *function, int line)
{
	...
	ASSERT_EQ (Ok, status);
    ASSERT_EQ (expectedCount / sizeof (GpRectF), count);
...
ATTRIBUTE_USED static void verifyRegionDataImpl (GpRegion *region, BYTE *expected, INT expectedCount, const char *file, const char *function, int line)
{
	...
	ASSERT_EQ (Ok, status);
    ASSERT_EQ (expectedCount, dataSize);
	...
	ASSERT_EQ (expectedCount, sizeFilled);
}
